...
record BlurpSnapshotData(
    ...
    static BlurpSnapshotData create(String sourceWorld, String label, Map<String, Map<Long, BlurpCompressedChunk>> stores) {
        ...
        BlurpSnapshotData snapshot = new BlurpSnapshotData(
            UUID.randomUUID(), sourceWorld, label, Instant.now(), Map.copyOf(stores), chunkCount, compressedBytes, uncompressedBytes, ""
        );
    ...
    BlurpSnapshotData withSha256(String sha256) {
        return new BlurpSnapshotData(
            this.id, this.sourceWorld, this.label, this.createdAt, this.stores,
